...
class MongoDB:
    ...
    def __new__(cls, uri: str, db_name: str):
        if (uri, db_name) not in cls._instances:
            ...
            instance.verification_data = instance.db['verification']  # New collection for verification records
    ...
    async def create_verification_record(self, user_id: int, file_id: str):
        """Create a new verification record for a file"""
        verification_key = f"verification_{user_id}_{file_id}"
        verification_data = {
            "_id": verification_key,
            "user_id": user_id,
            "file_id": file_id,
            "current_step": 0,
            "token_1": None,
    ...
    async def get_verification_record(self, user_id: int, file_id: str):
        """Get verification record for a file"""
        return await self.verification_data.find_one({"_id": f"verification_{user_id}_{file_id}"})

    async def update_verification_record(self, user_id: int, file_id: str, updates: dict):
